...
page_fault (struct intr_frame *f) 
{
  ...
  user = (f->error_code & PF_U) != 0;
   ...
   /* VM complement */
   void *esp_dummy;
   struct vm_entry *supt = NULL;

   /* Get corresponding esp pointer */
   if(user){esp_dummy = f->esp;}
   else{esp_dummy = thread_current()->esp;}
   
   /* If page isn't in dir */
   if(not_present){handle_fault(fault_addr,supt,esp_dummy);}
   else
   {
      thread_current()->exit_num = -1;thread_exit();
   }
